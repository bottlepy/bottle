...
class TestSimpleTemplate(unittest.TestCase):
    ...
    def test_unicode(self):
        ...
        t = SimpleTemplate('start {{var}} end').render(var='äöü')
        self.assertEqual('start äöü end', t)

    def test_unicode_code(self):
        """ Templates: utf8 code in file"""
        t = SimpleTemplate(name='./views/stpl_unicode.tpl').render(var='äöü')
        self.assertEqual('start ñç äöü end\n', t)
    ...
    def test_import(self):
        ...
        self.assertEqual('start dmFy end', t)

    def test_data(self):
        """ Templates: Data representation """
        t = SimpleTemplate('<{{var}}>')
        self.assertEqual('<True>', t.render(var=True))
        self.assertEqual('<False>', t.render(var=False))
        self.assertEqual('<None>', t.render(var=None))
        self.assertEqual('<0>', t.render(var=0))
        self.assertEqual('<5>', t.render(var=5))
        self.assertEqual('<b>', t.render(var='b'))
        self.assertEqual('<1.0>', t.render(var=1.0))
        self.assertEqual('<[1, 2]>', t.render(var=[1,2]))

    def test_escape(self):
        t = SimpleTemplate('<{{var}}>')
        self.assertEqual('<b>', t.render(var='b'))
        self.assertEqual('<&lt;&amp;&gt;>', t.render(var='<&>'))
    ...
    def test_newline_in_parameterlist(self):
        ...
        self.assertEqual("2", t.render())

    def test_dedentbug(self):
        ''' One-Line dednet blocks should not change indention '''
        t = SimpleTemplate('%if x: a="if"\n%else: a="else"\n{{a}}')
        self.assertEqual("if", t.render(x=True))
        self.assertEqual("else", t.render(x=False))
    ...
    def test_onelinebugs(self):
        ...
        t = SimpleTemplate('%if x:\n%a=1\n%end\n{{a}}')
        self.assertEqual("1", t.render(x=True))
        t = SimpleTemplate('%if x: a=1\n{{a}}')
        self.assertEqual("1", t.render(x=True))
        t = SimpleTemplate('%if x:\n%a=1\n%else:\n%a=2\n%end\n{{a}}')
        self.assertEqual("1", t.render(x=True))
        self.assertEqual("2", t.render(x=False))
        t = SimpleTemplate('%if x:   a=1\n%else:\n%a=2\n%end\n{{a}}')
        self.assertEqual("1", t.render(x=True))
        self.assertEqual("2", t.render(x=False))
        t = SimpleTemplate('%if x:\n%a=1\n%else:   a=2\n{{a}}')
    ...
    def test_escaped_codelines(self):
        ...
        self.assertEqual('% test', t.render())
        t = SimpleTemplate('%%% test')
        self.assertEqual('%% test', t.render())
    ...
    def test_detect_pep263(self):
        ...
        t = SimpleTemplate('%#coding: iso8859_15\nöäü?@€'.encode('utf8'))
        self.failIfEqual('öäü?@€', t.render())
        self.assertEqual(t.encoding, 'iso8859_15')
        t = SimpleTemplate('%#coding: iso8859_15\nöäü?@€'.encode('iso8859_15'))
        self.assertEqual('öäü?@€', t.render())
        self.assertEqual(t.encoding, 'iso8859_15')
    ...
    def test_ignore_pep263_in_textline(self):
        ...
        self.assertRaises(UnicodeError, lambda: SimpleTemplate('#coding: iso8859_15\nöäü?@€'.encode('iso8859_15')).co)
        t = SimpleTemplate('#coding: iso8859_15\nöäü?@€'.encode('utf8'))
        self.assertEqual('#coding: iso8859_15\nöäü?@€', t.render())
        self.assertEqual(t.encoding, 'utf8')

    def test_ignore_late_pep263(self):
        ''' PEP263 strings must appear within the first two lines '''
        self.assertRaises(UnicodeError, lambda: SimpleTemplate('\n\n%#coding: iso8859_15\nöäü?@€'.encode('iso8859_15')).co)
        t = SimpleTemplate('\n\n%#coding: iso8859_15\nöäü?@€'.encode('utf8'))
        self.assertEqual('\n\nöäü?@€', t.render())
        self.assertEqual(t.encoding, 'utf8')
