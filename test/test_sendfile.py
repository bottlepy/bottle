...
class TestSendFile(unittest.TestCase):
    ...
    def test_invalid(self):
        ...
        f = static_file(
            os.path.join('./../', os.path.basename(__file__)), root='./views/')
    ...
    def test_mime(self):
        ...
        self.assertTrue(f.headers[
                        'Content-Type'].split(';')[0] in ('application/x-python-code', 'text/x-python'))
        f = static_file(
            os.path.basename(__file__), root='./', mimetype='some/type')
        self.assertEqual('some/type', f.headers['Content-Type'])
        f = static_file(
            os.path.basename(__file__), root='./', mimetype='text/foo')
        self.assertEqual('text/foo; charset=UTF-8', f.headers['Content-Type'])
        f = static_file(
            os.path.basename(__file__), root='./', mimetype='text/foo', charset='latin1')
    ...
    def test_ims(self):
        ...
        request.environ['HTTP_IF_MODIFIED_SINCE'] = time.strftime(
            "%a, %d %b %Y %H:%M:%S GMT", time.gmtime(100))
        self.assertEqual(open(__file__, 'rb').read(), static_file(
    ...
    def test_range_parser(self):
        ...
        self.assertEqual(
            [(10, 100), (90, 100), (5, 11)],  r('bytes=10-,-10,5-10'))
