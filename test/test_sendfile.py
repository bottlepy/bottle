...
class TestSendFile(unittest.TestCase):
    def test_valid(self):
        ...
        try: send_file(os.path.basename(__file__), root='./')
        except BreakTheBottle, e:
            self.assertEqual(open(__file__).read(), e.output.read())

    def test_invalid(self):
        """ SendFile: Invalid requests"""
        try: send_file('not/a/file', root='./')
        except HTTPError, e:
            self.assertEqual(404, e.http_status)
        try: send_file(os.path.join('./../', os.path.basename(__file__)), root='./views/')
        except HTTPError, e:
            self.assertEqual(401, e.http_status)
        try:
            fp, fn = tempfile.mkstemp()
            os.chmod(fn, 0)
            try: send_file(fn, root='/')
            except HTTPError, e:
                self.assertEqual(401, e.http_status)
        finally:
            os.close(fp)
    ...
    def test_mime(self):
        ...
        try: send_file(os.path.basename(__file__), root='./', mimetype='some/type')
        except BreakTheBottle, e:
            self.assertEqual('some/type', response.content_type)
        try: send_file(os.path.basename(__file__), root='./', guessmime=False)
        except BreakTheBottle, e:
