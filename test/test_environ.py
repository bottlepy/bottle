...
class TestRequest(unittest.TestCase):
    ...
    def test_script_name(self):
        ...
        for raw, norm in tests:
            self.assertEqual(
                norm, BaseRequest({'SCRIPT_NAME': raw}).script_name)
        # Strange paths
        tests = [('///', '/'), ('///x///', '/x/')]
        for raw, norm in tests:
            self.assertEqual(
...
class TestResponse(unittest.TestCase):
    ...
    def test_set_status(self):
        ...
        # Illegal, but acceptable three digit code
        rs.status = '999 Who knows?'
        self.assertEqual(rs.status, rs.status_line)
        self.assertEqual(rs.status_code, 999)
    ...
    def test_set_cookie_expires(self):
        ...
        r.set_cookie(
            'name2', 'value', expires=datetime.datetime(1970, 1, 1, 0, 0, 43))
        cookies = sorted([value for name, value in r.headerlist
                          if name.title() == 'Set-Cookie'])
        self.assertEqual(
...
class TestRedirect(unittest.TestCase):
    ...
    def test_relative_path(self):
        ...
        self.assertRedirect(
            './foo/test.html', 'http://127.0.0.1/foo/test.html')
